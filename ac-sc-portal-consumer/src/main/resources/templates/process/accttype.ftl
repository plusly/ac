...
.seltype {
 ...
 <div class="navbar-wrapper">
    ...
    <form id="accttypeForm" method="post">
		<button id="doApplyBtn" type="button" class="btn btn-danger btn-lg btn-block">认证申请</button>
	</form>
    </div> <!-- /container -->
      <!-- /END THE FEATURETTES -->
	...
	<script>
		...
		$("#accttypeForm").click(function(){
			if($(".seltype").length == 0){
				layer.msg("别走！选一个再走！");
				return ;
			}
			
			var path = "/consumer/process/apply/accttype/"+accttype;
			$("#accttypeForm").attr("action", path).submit();
		})
	</script>
  </body>
</html>